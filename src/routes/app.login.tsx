...
function LoginPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const res = mode === "masuk" ? await signInWithPassword(email, password) : await signUp(email, password, nama);
    setLoading(false);
      ...
      <div className="flex flex-col items-center justify-center px-5 py-10 md:px-10">
        <div className="w-full max-w-md">
          ...
          <Tabs value={mode} onValueChange={(v) => setMode(v as typeof mode)} className="mt-6">
            ...
            <TabsContent value={mode} className="mt-6 space-y-4">
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-xs uppercase"><span className="bg-background px-2 text-muted-foreground">atau email</span></div>
              </div>
              <form onSubmit={submit} className="space-y-4">
                {mode === "daftar" && (
                  <div className="space-y-1.5">
                    <Label htmlFor="nama">Nama lengkap</Label>
                    <Input id="nama" value={nama} onChange={(e) => setNama(e.target.value)} autoComplete="name" />
                  </div>
                )}
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} autoComplete={mode === "masuk" ? "current-password" : "new-password"} />
                </div>
                <Button type="submit" className="w-full" disabled={loading}>
                  {loading ? "Memproses…" : mode === "masuk" ? "Masuk" : "Daftar"} <ArrowRight className="h-4 w-4" />
                </Button>
              </form>
            </TabsContent>
          </Tabs>
          ...
          <p className="mt-6 rounded-xl border border-dashed border-border/70 bg-surface-soft/50 p-3 text-center text-xs text-muted-foreground">
            🔒 Akun baru otomatis berperan sebagai Orang Tua. Peran Guru/Admin diberikan oleh admin sekolah.
          </p>
        </div>
      </div>
    </div>
