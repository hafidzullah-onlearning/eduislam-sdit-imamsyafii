...
function NotifPage() {
  ...
  const markAll = () => {
    patch("notifikasi", (items) =>
      items.map((n) => (n.role === session?.role ? { ...n, dibaca: true } : n)),
    );
  };
  return (
    <div className="space-y-6">
      <PageHeader
        title="Notifikasi"
        description="Semua pemberitahuan terkait aktivitas sekolah."
        actions={
        ...
        <div className="space-y-2">
          {list.map((n) => (
            <Link
              ...
              className={`block rounded-2xl border p-4 shadow-soft transition hover:border-primary/40 ${n.dibaca ? "border-border/60 bg-card" : "border-primary/30 bg-primary/5"}`}
            >
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {formatDistanceToNow(new Date(n.tanggal), {
                      locale: idLocale,
                      addSuffix: true,
                    })}
                  </p>
