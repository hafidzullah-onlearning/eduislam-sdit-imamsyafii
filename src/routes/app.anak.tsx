...
function AnakPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Anak Saya" description="Pilih anak untuk melihat progres akademik dan tahfidz." />
      {anak.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {anak.map((a) => {
            ...
            return (
              <div key={a.id} className={`rounded-2xl border-2 p-5 shadow-soft transition ${active ? "border-primary bg-primary/5" : "border-border/60 bg-card hover:border-primary/40"}`}>
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">NIS {a.nis} • {a.jenisKelamin === "L" ? "Laki-laki" : "Perempuan"}</p>
                    <p className="mt-1 text-xs text-muted-foreground">Kelas {k?.nama} • {k?.tahunAjaran}</p>
                  </div>
                </div>
                <div className="mt-4 flex gap-2">
                  <Button size="sm" variant={active ? "default" : "outline"} className="flex-1" onClick={() => setActiveSiswa(a.id)}>
                    {active ? "Sedang aktif" : "Pilih anak ini"}
                  </Button>
                  <Link to="/app/dashboard"><Button size="sm" variant="ghost"><ArrowRight className="h-4 w-4" /></Button></Link>
                </div>
              </div>
            );
          })}
