...
function LaporanPage() {
  ...
  const myTahfidz = useMemo(() => tahfidz.filter((t) => mySiswa.some((s) => s.id === t.siswaId)), [tahfidz, mySiswa]);
  const tahfidzTotal = myTahfidz.length;
  const tahfidzLancar = myTahfidz.filter((t) => t.status === "lancar").length;

  const myPerilaku = useMemo(() => perilaku.filter((p) => mySiswa.some((s) => s.id === p.siswaId)), [perilaku, mySiswa]);
  ...
  const avgKarakter = pKarakter.length ? pKarakter.reduce((a, b) => a + b.skor, 0) / pKarakter.length : 4.0;
  const avgDisiplin = pDisiplin.length ? pDisiplin.reduce((a, b) => a + b.skor, 0) / pDisiplin.length : 4.0;
  const avgKeaktifan = pKeaktifan.length ? pKeaktifan.reduce((a, b) => a + b.skor, 0) / pKeaktifan.length : 4.0;
  const avgKerjasama = pKerjasama.length ? pKerjasama.reduce((a, b) => a + b.skor, 0) / pKerjasama.length : 4.0;

  const myMoods = useMemo(() => mood.filter((m) => mySiswa.some((s) => s.id === m.siswaId) && m.sumber === "sekolah"), [mood, mySiswa]);
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <div className="rounded-2xl border border-border/60 bg-card p-5 shadow-soft flex flex-col justify-between">
          <div className="space-y-4">
            ...
            <p className="text-xs text-muted-foreground">Analisis otomatis performa akademik, tahfidz, perilaku, dan emosi siswa untuk masukan guru.</p>
          </div>
          ...
          <div className="mt-4 flex-1 overflow-y-auto max-h-[250px] border border-border/60 rounded-xl bg-surface-soft/10 p-3.5">
            {saranAI ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground py-6">
                ...
                <p className="text-[11px] leading-normal px-4">Klik tombol di atas untuk menganalisis data kelas secara otomatis menggunakan Gemini AI.</p>
              </div>
            )}
          </div>
        </div>
