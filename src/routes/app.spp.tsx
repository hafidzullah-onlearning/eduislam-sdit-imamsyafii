...
function SPPPage() {
  const loading = useLazyLoadTables(["invoice"]);
  const { user } = useAuth();
  ...
  return (
    <div className="space-y-6">
      ...
      {list.length === 0 ? (
        <EmptyState
          icon={CheckCircle2}
          title={tab === "aktif" ? "Alhamdulillah, tidak ada tagihan" : "Belum ada riwayat"}
        />
      ) : (
          ...
          {list.map((inv) => {
            ...
            return (
              <Link
                ...
                to="/app/spp/$id"
                params={{ id: inv.id }}
                className="block rounded-2xl border border-border/60 bg-card p-5 shadow-soft transition hover:border-primary/50"
              >
