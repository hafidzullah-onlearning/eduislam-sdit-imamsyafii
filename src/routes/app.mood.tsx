  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={isGuru ? "Catat suasana hati siswa per jam pelajaran." : "Bagaimana perasaan anak Anda hari ini?"}
      />
      ...
      <div className="rounded-2xl border border-border/60 bg-card p-5 shadow-soft">
          ...
          <div className="mb-3 grid gap-3 sm:grid-cols-2">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={jam} onValueChange={setJam}>
                ...
                <SelectContent>{[1, 2, 3, 4, 5, 6, 7, 8].map((n) => <SelectItem key={n} value={String(n)}>Jam ke-{n}</SelectItem>)}</SelectContent>
              </Select>
            </div>
          </div>
        )}
          ...
          {EMOJI.map((e) => (
            <button
              ...
              className={`flex flex-col items-center gap-1.5 rounded-2xl border-2 p-4 transition ${
                selected === e.id ? "border-primary bg-primary/10" : "border-border/60 hover:border-primary/40"
              }`}
            >
        ...
        <div className="mt-3 space-y-1.5">
          ...
          <Textarea value={catatan} onChange={(e) => setCatatan(e.target.value)} placeholder="Contoh: Setelah mengerjakan PR bersama, Faris terlihat senang." />
        </div>
        <Button className="mt-3" onClick={submit}>Simpan mood</Button>
      </div>
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer>
              <LineChart data={chart}>
                ...
                <Line type="monotone" dataKey="Sekolah" stroke="var(--color-primary)" strokeWidth={2.5} dot={{ r: 3 }} connectNulls />
                <Line type="monotone" dataKey="Rumah" stroke="var(--color-gold)" strokeWidth={2.5} dot={{ r: 3 }} connectNulls />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      {list.length === 0 ? (
        ...
        <div className="rounded-2xl border border-border/60 bg-card p-5 shadow-soft">
          ...
          <div className="space-y-2">
            {list.slice().reverse().slice(0, 15).map((m) => (
              <div key={m.id} className="flex items-center justify-between gap-2 rounded-xl border border-border/60 p-3">
                <div className="flex items-center gap-3">
                  <span className="text-2xl">{EMOJI.find((e) => e.id === m.emoji)?.icon}</span>
                  <div>
                    <p className="text-sm font-semibold capitalize">{m.emoji} <span className="text-xs font-normal text-muted-foreground">• {m.sumber === "sekolah" ? "Sekolah" : "Rumah"}{m.jamPelajaran && ` jam ke-${m.jamPelajaran}`}</span></p>
                    <p className="text-xs text-muted-foreground">{formatDistanceToNow(new Date(m.tanggal), { locale: idLocale, addSuffix: true })}</p>
                    {m.catatan && <p className="mt-1 text-xs italic text-muted-foreground">"{m.catatan}"</p>}
                  </div>
                </div>
              </div>
            ))}
